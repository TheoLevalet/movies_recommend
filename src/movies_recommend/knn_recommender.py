...
class KnnRecommender:
    ...
    def _prep_data(self):
        ...
        df_movies_cnt = pd.DataFrame(
            df_ratings.groupby("movieId").size(), columns=["count"]
        )
        popular_movies = list(
            set(df_movies_cnt.query("count >= @self.movie_rating_thres").index)
        )  # noqa
        ...
        active_users = list(
            set(df_users_cnt.query("count >= @self.user_rating_thres").index)
        )  # noqa
        ...
        hashmap = {
            ...
            for i, movie in enumerate(
                list(df_movies.set_index("movieId").loc[movie_user_mat.index].title)
            )  # noqa
        }
    ...
    def make_recommendations(self, fav_movie, n_recommendations):
        ...
        raw_recommends = self._inference(
            self.model, movie_user_mat_sparse, hashmap, fav_movie, n_recommendations
        )
        # print results
...
def parse_args():
    parser = argparse.ArgumentParser(
        prog="Movie Recommender", description="Run KNN Movie Recommender"
    )
    parser.add_argument(
        "--path", nargs="?", default="./assets/", help="input data path"
    )
    parser.add_argument(
        "--movies_filename",
        nargs="?",
    ...
    parser.add_argument(
        "--movie_name", nargs="?", default="", help="provide your favoriate movie name"
    )
    parser.add_argument(
        "--top_n", type=int, default=10, help="top n movie recommendations"
    )
